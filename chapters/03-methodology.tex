...
The evaluation employs a bottom-up approach, progressing from isolated microbenchmarks to a full application context. This progression is structured in three stages:

\begin{enumerate}
    \item \textbf{Isolated Microbenchmarks:} Used to identify the optimal message sizes and topological configurations for a given backend.
    \item \textbf{Composite \texttt{cg\_step}:} Evaluates whether baseline observations hold true in a more complex sequence that combines halo exchanges, local computation, and scalar reductions.
...
\toprule
Stack & Ping-pong & Halo & Allreduce & All-to-all & \texttt{cg\_step}\\
\midrule
\texttt{cuda\_mpi} & Yes & Yes & Yes & Yes & Yes\\
\texttt{sycl\_mpi} & Yes & Yes & Yes & Yes & Yes\\
\texttt{cuda\_nccl} & Yes & Yes & Yes & Yes & Yes\\
\texttt{cuda\_nvshmem} & Yes & Yes & Yes & Yes & Yes\\
\texttt{oshmpi} & Yes & Yes & Yes & Yes & Yes\\
\texttt{sycl\_oneccl} & Yes & Yes & Yes & Yes & Yes\\
\texttt{sycl\_oneccl\_oshmpi} & No & No & Yes & Yes & No\\
\bottomrule
\end{tabular}
\end{table}
...
Ping-pong uses \texttt{1n2g} and \texttt{2n1g}. Halo adds \texttt{1n4g}, \texttt{2n4g}, \texttt{4n4g}, and \texttt{8n4g}; collectives and the composite step also retain \texttt{1n1g} controls. Table~\ref{tab:m-pattern-coverage} gives the exact backend matrix: oneCCL/OSHMPI has standalone allreduce and all-to-all data but no ping-pong, halo, or composite-step campaign. The suite also implements variable-count MoE traffic, but no complete MoE campaign is available for quantitative conclusions.
