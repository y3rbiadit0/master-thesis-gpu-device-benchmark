...
Placement & Node and GPU count; one rank drives one GPU & Seven, \texttt{1n1g}--\texttt{8n4g}\\
...
\midrule
Trials per allocation & Process launches sharing one placement & 3\\
Independent allocations & Scheduler jobs with separate placements & 5; 3 for \texttt{alltoall} and \texttt{cg\_step}\\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{What is measured.} The first four parameters identify a \emph{cell}: one $(\text{pattern}, \text{stack}, \text{placement})$ combination, swept over the payload axis. Placement is expressed as $N$ nodes by $G$ GPUs rather than as a single rank count, because two configurations with equal rank counts can use entirely different interconnects; \texttt{1n4g} and \texttt{2n1g} are the deliberate pair that separates those effects. The collective policy that applies to a pattern, such as whether UCC is enabled, is fixed per pattern rather than chosen per point, and is described in Section~\ref{sec:runtime-method}. A few patterns add their own controls, such as the halo's isolated and steady-state sample counts, \texttt{cg\_step}'s phase pass, and MoE's routing cases.
