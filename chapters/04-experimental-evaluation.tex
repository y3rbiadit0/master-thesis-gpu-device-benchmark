...
\caption{Allreduce latency for all seven measured stacks at representative topologies. Points are medians of allocation means and shading spans those means. The input payload is per rank; the single-rank control is discussed separately.}
...
All-to-all gives the clearest example of a configuration changing the apparent library ranking, and its UCC profile is the opposite of allreduce's. Figure~\ref{fig:m-alltoall-backends} first shows all seven measured stacks against bytes per peer, rather than the topology-dependent aggregate send-buffer size. The MPI default changes regime sharply as rank count and per-peer size increase, while oneCCL/OSHMPI's staged path remains substantially more expensive.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/results/alltoall-backends.png}
\caption{All-to-all latency for all seven measured stacks at representative topologies. The horizontal axis is payload per peer; points are medians of allocation means and shading spans those means.}
...
Allreduce shows the second effect, and it runs in the opposite direction. At 16\,MiB, \texttt{2n1g} costs 1557\,$\mu$s for NCCL even though only two GPUs participate, whereas \texttt{8n4g} with 32 GPUs costs 1104\,$\mu$s. Adding intra-node ranks does not simply add work: the NVLink stage of a hierarchical reduction shrinks the payload that must cross InfiniBand. The same mechanism explains why the NCCL-based paths pull away from MPI only once each node holds four GPUs, from 567\,$\mu$s against 1118\,$\mu$s at \texttt{2n4g} to 1104\,$\mu$s against 1879\,$\mu$s at \texttt{8n4g}. In the scalar regime, where no such staging is available, the four-byte allreduce instead grows monotonically with rank count, from roughly 18--20\,$\mu$s at \texttt{1n4g} to 46--49\,$\mu$s at \texttt{8n4g} for every path except NVSHMEM, whose 88\,$\mu$s belongs with its separated multi-node behavior.
...
\midrule
Ping-pong & 46 & $1.01$ & $1.84$ & $1.10$\\
Halo, isolated & 126 & $1.02$ & $1.19$ & $1.32$\\
Halo, steady & 126 & $1.01$ & $1.07$ & $1.13$\\
Allreduce & 161 & $1.00$ & $1.06$ & $1.02$\\
All-to-all & 119 & $1.01$ & $1.06$ & $1.02$\\
MoE & 21 & $1.00$ & $1.08$ & $1.02$\\
\midrule
...
For the five patterns that time communication alone, SYCL costs about 1\% at the median and stays within 8\% in all but one case. Ping-pong is the exception, where the largest messages reach $1.84\times$; small messages there sit in the MPI startup regime, which both stacks share, while the large-message endpoints diverge. On this evidence, expressing a GPU-aware MPI exchange through SYCL rather than CUDA is close to free, which is the expected result: the payload path, transport, and completion boundary are identical, and only the host-side submission and the buffer's allocator change.
...
\begin{table}[htbp]
\centering
\caption{MoE dispatch-and-combine latency in \,$\mu$s, medians of allocation means, contrasting the largest intra-node topology with the largest multi-node topology for each routing case.}
\label{tab:m-moe}
\footnotesize
...
Two qualifications bound these results. MoE keeps UCC disabled and uses the variable-count Open MPI path, so the MPI series is one fixed configuration rather than a tuned result, and the all-to-all tuning of Section~\ref{sec:micro-results} does not transfer to it untested. Second, the reported volume is useful application bytes including tokens that never leave their source rank, so the \texttt{locality80} latencies correspond to much less traffic than their byte count suggests; they are comparable across stacks, not convertible into a link rate.

\subsection{Comparison with All-to-All}

The standalone all-to-all sweep of Section~\ref{sec:micro-results} moves equal blocks between all peers, and on that evidence the NCCL-based paths own the bulk regime. MoE retains the same operation family and the same topologies, changes only the distribution of block sizes, and inverts that conclusion in two of three cases. The regime boundary that matters for this workload is therefore not the payload size that the primitive sweep varies, but the skew that it holds constant.
