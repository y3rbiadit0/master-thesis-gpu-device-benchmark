...
\subsubsection{MPI with UCC: A Size-Dependent Trade-off}
...
\subsubsection{MPI with UCC: The Opposite Trade-off}
...
\subsection{Scaling Up and Scaling Out}\label{sec:micro-scaling}
...
\subsection{The Portability Cost of SYCL Is Small in Communication}\label{sec:sycl-portability}
...
\section{Routing-Dependent Redistribution in the \texttt{moe} Benchmark}\label{sec:moe-results}
...
\subsection{Placement Scaling Under Three Routing Cases}
...
\subsection{The Ranking Depends on the Routing Case}
...
\subsection{What MoE Adds to the Primitive Evidence}
...
\section{NVSHMEM Across the Scale Boundary}\label{sec:nvshmem-overlook}
...
These are observations about the configured path as measured. Section~\ref{sec:ibgda-check} already records that inter-node device-initiated requests are serviced by a CPU proxy rather than posted by the GPU, and Chapter~\ref{ch:m-discussion} examines how far that, the collective dispatch configuration, and the changed execution model account for the trend.
