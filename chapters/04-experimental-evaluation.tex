...
Reading the four patterns across the placement axis rather than the size axis separates two different effects: losing NVLink, and adding GPUs. The steady-state halo at a 16\,MiB aggregate payload makes the first effect explicit. Within one node the aggregate rate is 325\,GB/s for OSHMPI and 280\,GB/s for CUDA MPI at \texttt{1n4g}. Moving the same exchange to \texttt{2n1g}, which keeps two ranks but places them on separate nodes, drops every stack to 20--24\,GB/s, an order-of-magnitude change produced by the interconnect alone. The rate then stays essentially flat as nodes are added: NVSHMEM holds 41.1, 40.9, and 40.6\,GB/s at \texttt{2n4g}, \texttt{4n4g}, and \texttt{8n4g}, and CUDA MPI holds 33.9--36.5\,GB/s over the same range. Once the exchange is inter-node, its aggregate rate is limited by what one node can inject, not by the number of nodes participating.
...
The MoE archive is complete for the six declared stacks across all seven placements and all three routing cases, with five allocations of three trials per cell. Total useful volume is held fixed at 32\,MiB per rank for dispatch and combine together, so a difference between routing cases is a difference in distribution, not in bytes moved.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/results/moe-routing-scaling.png}
\caption{MoE dispatch-and-combine latency for the six measured stacks at every placement, under the three routing cases. Points are medians of five allocation means and shading spans those means. The dotted line separates intra-node NVLink placements from those crossing InfiniBand. At \texttt{1n1g} all three cases degenerate to a local copy.}
...
\begin{table}[htbp]
\centering
\caption{MoE dispatch-and-combine latency in \,$\mu$s, medians of allocation means, contrasting the largest intra-node placement with the largest multi-node placement for each routing case.}
\label{tab:m-moe}
\footnotesize
...
\caption{MoE latency at \texttt{8n4g} relative to CUDA MPI at the same placement and routing case. Bars below one favor the compared stack. No stack is below one in all three cases.}
