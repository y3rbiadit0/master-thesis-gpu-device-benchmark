...
The \texttt{cuda\_nvshmem\_device} stack issues the halo exchange and the scalar reductions from inside the kernel, so it is the benchmark-level counterpart of the monolithic device solver. It covers every topology and seven grid sides from 256 to 16\,384, with three allocations of three trials per cell, except at \texttt{8n4g}, where the smallest and largest sides are missing. Because the device implementation reports no phase timers, it appears in step time only and not in the phase breakdown below.
