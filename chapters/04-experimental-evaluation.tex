...
The results follow the three levels defined in Section~\ref{sec:research-planning-levels}, and develop the selection argument in three steps. First, the isolated microbenchmarks identify candidate stacks and the configurations that make them competitive, separately within one node and across nodes. Second, the two application microbenchmarks test those candidates under a workload's communication sequence: \texttt{moe} keeps the operation but replaces the uniform payload distribution with routing-dependent block sizes, and \texttt{cg\_step} adds local computation and scalar reductions. Third, aCG determines which observations survive an irregular sparse partition, synchronization dependencies, and convergence. The common-interface results explain what is required to expose these paths through oneCCL.

Every section reports the same topology progression: \texttt{1n2g} and \texttt{1n4g} on intra-node NVLink, \texttt{2n1g} as a single InfiniBand link without intra-node traffic, and \texttt{2n4g} through \texttt{8n4g} at 8, 16, and 32 GPUs with both levels active. The recurring question is whether a stack's intra-node advantage survives the crossing of that boundary.
...
For reductions, the dominant distinction is between startup-sensitive scalar work and bulk combination. Figure~\ref{fig:m-allreduce-backends} shows all seven measured paths at representative intra- and multi-node topologies. NCCL-based paths lead the large-message regime (Table~\ref{tab:m-allreduce-bandwidth}). Within one node, NVSHMEM matches NCCL closely, at 128.3 against 127.5\,GB/s on \texttt{1n4g}: NVSHMEM normally dispatches these benchmark collectives through NCCL, so that agreement is evidence about one configured path rather than two independent collective algorithms.
