...
The results describe complete configured stacks on one system (Section~\ref{sec:eval-environment}), with inter-node NVSHMEM requests serviced by a CPU proxy (Section~\ref{sec:ibgda-check}), two sparse matrices for the solver, and redistribution exercised only at microbenchmark level. What is expected to transfer is the set of regime boundaries and the conditions that move them, not absolute times or orderings. Within that scope, three limits qualify the claims. First, no comparison varies one factor at a time: the device-invoked NVSHMEM variants change persistence, batching, and signaling together with invocation, so the regime in which device initiation pays is established but its mechanism is not. Second, the two cross-level agreement checks are descriptive and cover five rank counts and two grid sides, so the conditions for transfer are shown to be necessary in the observed cases, not sufficient in general. Third, each cell holds five independent allocations, three for \texttt{alltoall} and \texttt{cg\_step}, and no hypothesis tests are computed; small differences are therefore not read as orderings, and only order-of-magnitude separations and regime boundaries are relied on. The exploratory NCCL measurements in Appendix~\ref{app:nccl-preliminary} lie outside these claims.
