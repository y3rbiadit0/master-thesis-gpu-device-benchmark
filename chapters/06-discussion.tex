...
This chapter interprets the measurements in Chapter~\ref{ch:m-results} in terms of when GPU-initiated communication benefits an application and which communication patterns support that benefit. It first connects the primitive, composite, and conjugate gradient results, then examines NVSHMEM's behavior across patterns and topologies. The resulting synthesis answers the research questions and establishes the limits of those answers, with particular attention to completion dependencies, progress mechanisms, and differences between the evaluated implementations.
...
These limitations motivate the four research directions in Section~\ref{sec:priority-experiments}: validation on IBGDA-enabled clusters, isolation of initiation effects, stronger benchmark--application correspondence, and evaluation of newer device-communication interfaces.
